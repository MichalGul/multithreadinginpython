...
def parse_to_array(text_conn: Connection, metars_conn: Connection):
    text = text_conn.recv()
...
def extract_wind_direction(metars_conn: Connection, winds_conn: Connection):
...
def mine_wind_distribution(winds_conn:Connection, wind_dist_conn:Connection):
    wind_dist = [0] * 8
...
if __name__ == '__main__':
    ...
    winds_dis_conn_a, winds_dis_conn_b = Pipe()
    ...
    Process(target=mine_wind_distribution, args=(winds_conn_b,  winds_dis_conn_a)).start()
    ...
    wind_dist = winds_dis_conn_b.recv()
    end = time.time()
    print(wind_dist)
